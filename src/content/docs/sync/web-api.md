...
Syncs a single member from Sportlink to Rondo Club. Launches a Playwright browser session to fetch fresh data from Sportlink (person details, parents, functions, committees, and free fields), then syncs the member to WordPress. First, a targeted SearchMembers query looks up the exact KNVB ID with union team data enabled; an absent active match is retried with the inactive-member filter. The partial `/general` response is overlaid on this fresh result, so `KernelGameActivities` and `AgeClassDescription` no longer wait for the next bulk import. A missing or ambiguous member, an incomplete activity/age-class response, or a failed search aborts before person updates; an explicitly empty activity clears the previous value. Search navigation retries a stuck panel once and can refresh an expired session. Fresh parent data overrides the member's record from the latest full snapshot before parent deduplication, so a newly added parent is created or updated immediately and linked to the child in both directions. Other snapshot members remain available so shared parents retain links to siblings.
