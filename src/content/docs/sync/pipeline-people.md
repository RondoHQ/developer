...
8. Then processes **parent members** (from `rondo_club_parents` table):
   ...
   - Onboarding source checks use the parent IDs returned by the current save and verify both directions of each parent-child relationship. A confirmed parent alias can differ from an active member, contact, or sponsor's managed email. Standalone and former-member parent profiles must still contain the source email; missing mappings, missing reciprocal links, and failed saves leave the check incomplete.
...
| `spelactiviteit` | `KernelGameActivities`; repeated whitespace is normalized, an explicitly empty value becomes `null` and clears the previous value, while a partial response that omits the property leaves the existing value untouched |
