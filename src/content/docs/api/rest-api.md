...
Returns rolling 7- and 30-day payment totals, average payment lead time, installment-plan counts, 30 daily income buckets, 12 monthly income buckets, and `membership_payment_status` for the current season. The membership status always counts paid versus open contribution invoices and excludes drafts, cancelled invoices, and credits. Pass `invoice_type=membership|discipline|manual|volunteer_fine` to filter the other statistics by invoice type. Credit invoices are excluded because they are not income.
