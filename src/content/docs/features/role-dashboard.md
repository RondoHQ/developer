...
- Membership requires `ledenadministratie`. Active members exclude former members, future membership starts and ended membership dates. Instroom and uitstroom use `lid_sinds` and `lid_tot` from 1 July through today inclusive, including former members for departures. Future departures are not counted yet. These are date-based current records, not a historical snapshot or a net-growth calculation.
