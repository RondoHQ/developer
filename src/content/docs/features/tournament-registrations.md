...
Rondo manages tournament editions and one shared registration task per selected club team. The
first milestone covers invitations and positive registrations; payment links, reminders, external
submission tracking, and programme distribution are separate follow-up milestones.
...
to assigned accounts. The assignment stores a snapshot of names, roles, and email addresses plus
private per-user lookup markers for efficient personal task lists.
